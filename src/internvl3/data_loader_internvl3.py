...
class DeceptionDataset(Dataset):
    def __init__(self, video_label_map: Dict[str, int], tokenizer, video_dir, num_segments=16, max_num=4):
    ...
    def __getitem__(self, idx):
        ...
        pixel_values, num_patches_list = self.load_video(
            video_path, num_segments=self.num_segments, max_num=self.max_num)

        question = ''.join([f'Frame{i + 1}: <image>\n' for i in range(len(num_patches_list))])
        question += ('Your task is to detect if the person in this scene is lying. '
                     'Output "true" if you detect deception (person is lying), otherwise output "false".')

        answer = "true" if label == 1 else "false"
        prompt = question + "\nAnswer:"

        # Tokenize text with label appended as target
        encoding = self.tokenizer(prompt, text_target=answer, return_tensors='pt', padding="max_length",
                                  truncation=True)
